...
def novo_pedido():
    ...
    status = supabase.table("config").select("*").eq("chave", "loja_aberta").single().execute()
    if not status.data or status.data['valor'] != 'true':
        return jsonify({"error": "Loja fechada para pedidos"}), 403
...
def atualizar_status_pedido(id):
    data = request.json
    supabase.table("pedidos").update({"status": data.get("status")}).eq("id", id).execute()
